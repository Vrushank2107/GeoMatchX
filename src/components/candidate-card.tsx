...
export function WorkerCard({ 
  ...
  // Check recruitment status on mount and when candidate changes
  useEffect(() => {
    if (showRecruitButton && isSME && candidate?.id) {
      checkRecruitmentStatus();
    }
  }, [showRecruitButton, isSME, candidate?.id]);

  async function checkRecruitmentStatus() {
    try {
      const response = await fetch(`/api/candidate/recruitment-status?worker_id=${candidate.id}`);
      if (response.ok) {
  ...
  return (
    <Card className={cn("flex flex-col gap-4 group hover:scale-[1.02] transition-transform duration-300", className)}>
      <CardHeader className="gap-2">
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardTitle className="text-xl group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">{candidate.name}</CardTitle>
          </div>
          <div className="flex flex-col items-end text-sm">
            ...
            <span className="text-xs text-zinc-500">{candidate.experience}+ yrs exp.</span>
          </div>
        </div>
        <p className="text-sm text-zinc-600 dark:text-zinc-400 font-medium">{candidate.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <p className="text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed line-clamp-2">{candidate.bio}</p>
        <div className="flex items-center justify-between text-sm pt-2 border-t border-zinc-100 dark:border-zinc-800">
          ...
          <div className="flex items-center gap-2">
            ...
            {actionHref ? (
              ...
              <Button size="sm" variant="secondary" onClick={() => onAction?.(candidate)} className="shadow-sm">
                {actionLabel}
              </Button>
            )}
          </div>
