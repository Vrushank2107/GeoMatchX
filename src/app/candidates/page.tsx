...
export default function WorkersPage() {
  ...
  const [candidates, setWorkers] = useState<DirectoryWorker[]>([]);
  ...
  useEffect(() => {
    async function fetchWorkers() {
      try {
        ...
        // Filter out current user if they are a candidate
        if (isWorker && user) {
          const currentUserId = user.id;
          // Candidate IDs are in format "wkr-{user_id}", so we need to extract the numeric part
          filteredWorkers = filteredWorkers.filter((candidate: DirectoryWorker) => {
            const workerIdMatch = candidate.id.match(/wkr-(\d+)/);
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <p className="text-xs uppercase tracking-[0.4em] text-indigo-500">Verified directory</p>
            <h1 className="text-3xl font-semibold">
              {isSME ? "Operators ready for activation" : "Browse fellow candidates"}
            </h1>
        </div>
          {isSME && (
        <Button asChild>
        ...
        <div className="rounded-2xl border border-zinc-200 bg-zinc-50 p-8 text-center dark:border-zinc-800 dark:bg-zinc-900">
          <p className="text-zinc-500">No candidates found. Check back later!</p>
        </div>
      ) : (
