...
import { getWorkerById, type Candidate } from "@/lib/mockData";
...
export default function WorkerProfilePage() {
  ...
  const [candidate, setWorker] = useState<Candidate | null>(null);
      ...
      {isLoading ? (
        ...
        <div className="space-y-8">
          <Card className="border-amber-200 bg-amber-50 dark:border-amber-900 dark:bg-amber-950/50">
            <CardContent className="pt-6">
              <p className="text-amber-600 dark:text-amber-400 font-medium">Candidate not found</p>
      ...
      <div className="rounded-[32px] border border-white/40 bg-white/80 p-8 shadow-xl backdrop-blur dark:border-zinc-900 dark:bg-zinc-950/80">
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 rounded-full bg-indigo-50 px-4 py-2 text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-200">
            ...
            {candidate.rating.toFixed(1)} · {candidate.experience}+ yrs
          </div>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        <Card>
          <CardContent className="space-y-4 pt-6">
            <section>
              ...
              <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-300">{candidate.bio}</p>
            </section>
            <section>
        ...
        <Card>
          <CardContent className="space-y-4 pt-6">
            ...
            {isSME ? (
              <>
                <p className="text-sm text-zinc-500">Send a recruitment request to this candidate. They will be notified and can respond to your offer.</p>
                {recruitSuccess ? (
                  <div className="rounded-lg bg-green-50 p-4 dark:bg-green-950/50">
                    ...
                    <p className="text-sm text-green-600 dark:text-green-400 mt-2">
                      The candidate has been notified and will respond soon.
                    </p>
                  </div>
                ) : (
                  <Button 
                ...
                <div className="pt-4 border-t border-zinc-200 dark:border-zinc-800">
                  ...
                  <Button variant="outline" className="w-full" asChild>
                    <a href={`mailto:${candidate.email || 'cx@geomatchx.com'}`}>
